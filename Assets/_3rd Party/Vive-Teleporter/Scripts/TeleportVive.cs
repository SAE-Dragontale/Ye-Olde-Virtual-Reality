...
public class TeleportVive : MonoBehaviour {
	...
	public void AssignPointerMesh()
	{
		if(Pointer == null)
		{
			Pointer = FindObjectOfType<ParabolicPointer> ();
			Debug.Log ("I restarted pointer");
		}

		if(NavmeshAnimator == null)
		{
			NavmeshAnimator = FindObjectOfType<ViveNavMesh> ().gameObject.GetComponent<Animator> ();
			Debug.Log ("I restarted navanimator");
		}
	}
