...
public class ParabolicPointer : MonoBehaviour {
	...
	void Awake()
	{
		_sceneRoot = this.transform.root.gameObject;
	}
    ...
    void Start() {
        ...
        if(SelectionPadPrefab != null)
        {
			...
			SelectionPadObject.transform.SetParent (this.transform.root.gameObject.transform);
        }
        ...
        if (InvalidPadPrefab != null)
        {
			...
			InvalidPadObject.transform.SetParent (this.transform.root.gameObject.transform);
        }
    }
